...
const networkTitleMap = {
  ChinaUnicom: 'Baby',
  ChinaTelecom: 'babylove',
  ChinaMobile: '该死你用的移动',
};
...
!(async () => {
  ...
  if (typeof $network !== 'undefined') {
    ...
    networkOperator = $.lodash_get($network, 'operator');
  ...
  if (
    ...
    let title = networkTitleMap[networkOperator] || '网络信息';
    await notify(title, `${CN_IP} | ${PROXY_IP}`, `${CN_ADDR}`, `${PROXY_ADDR}\n${ssid || ''} ${primaryAddress || ''}`);
  } else {
    $.log('IP 相同 不发送通知');
  }
})()
